...
| Edwards/Montgomery conversion | X301 key generation reduces the clamped scalar modulo `L`, uses the existing constant-time fixed-base table, then maps directly as `u=(Z+Y)/(Z-Y)` with one field inversion and no affine encode/decode cycle. | Removes the 301-round basepoint ladder without adding arithmetic; the reduced scalar has the established fixed-base contract. | Nine scalar boundaries, byte equality with the retained ladder, 10,000-case Python differential, taint and final-provider codegen. |
