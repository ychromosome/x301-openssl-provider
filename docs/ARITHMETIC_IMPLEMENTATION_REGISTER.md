...
| --- | --- | --- | --- |
| Runtime field borrow | Safe `u64::borrowing_sub` chain | Replaces the larger Hacker's-Delight expansion after Rust 1.97.1 lowered every reviewed X301 use site to branchless SBB/CMOV code. The expansion originally repaired a real secret-dependent `overflowing_sub` lowering under older compilers. | Core differential/KAT tests, X301 taint cases and `scripts/check-x301-final-codegen.sh`. |
...
| Field small-product reduction and square | Local adaptations, 2026-09-05: omit impossible small-fold underflow correction; arrange square cross terms before diagonals following OpenSSL `bn_sqr_normal`. | Preserve the existing 5x64 backend and bounds with fewer correction instructions and different carry scheduling. | Bounds, source identities and tests: `ED301_ARITHMETIC_PROVENANCE.md`; taint and final codegen gates remain mandatory. |
