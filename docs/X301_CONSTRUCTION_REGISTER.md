...
| E4 | No new RNG route | OpenSSL provider RAND contract; existing provider keygen contract | Generate private material through provider RAND. Fixed-key derive consumes no RAND. Sole registered exception: X-RAND-INSTANCE below. |
...
| X-H1 | Order all three hybrid values ML-KEM first and name the group `X301MLKEM1024` | RFC 10024 Sections 4-5; OpenSSL `X448MLKEM1024` | Matches the established X-family names while retaining the RFC 10024 ML-KEM-first layout. | The obsolete `MLKEM1024X301` name is rejected; 1606/1606/70-byte size, offset-1568 delete/insert and parser tests remain unchanged. |
...
| X-RAND-INSTANCE | **Exception to E4 as originally worded; approval pending (2026-09-04).** X301 key generation draws from one provider-owned `EVP_RAND_CTX` (`CTR-DRBG`, `AES-256-CTR`, no parent, locking enabled) fetched under the child-context provider and property policy, created on first key generation and freed in provider teardown, instead of `RAND_priv_bytes_ex()` on the child context | OpenSSL `provider-rand(7)`/`EVP_RAND(3)`; core `get_user_entropy` seeding as used by OpenSSL's own providers, including the FIPS module, for their DRBG instances | `RAND_priv_bytes_ex(child)` registers, per thread, a thread-exit handler that dereferences the child context, and OpenSSL frees a context with only the calling thread's handlers; the only OpenSSL-provided mitigation is `OPENSSL_thread_stop_ex(child)` after every operation, which is not that function's intended use and cost 3.7x keygen throughput at 16 threads. The instance uses only OpenSSL algorithms and seeding; the project owns selection (`CTR-DRBG`, libcrypto's default), lazy creation, locking and teardown. Known semantics, identical to libcrypto's own DRBGs: the instance follows the property policy in force at first key generation; a later policy change applies to new library contexts only, not to a warm instance, and `OSSL_PROVIDER_unload()`/reload within one context keeps the instance because OpenSSL tears a provider down at its last reference. The DRBG type is fixed because a provider cannot read the application's `RAND_set_DRBG_type()` setting (the child context could not either). | T7 lifecycle test (teardown before worker exit; Valgrind negative control with the stops removed fails in `rand_delete_thread_state`), T7 fail-closed/recovery test (fresh context re-evaluates policy; unload/reload does not), T7 warm-instance policy test, unchanged T7 deterministic and poisoned RAND tests |
