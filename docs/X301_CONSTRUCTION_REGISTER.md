...
| --- | --- | --- | --- |
| E1 | ML-KEM-1024 is OpenSSL-owned | FIPS 203; OpenSSL EVP ML-KEM contract | Fetch through EVP in the provider child library context from providers visible through the application context. Child-local fallback is disabled. Without ML-KEM, raw X301 remains available but the hybrid capability is absent and hybrid operations fail. No Rust crate, vendored implementation, copied substep or project-owned ML-KEM symbol. |
...
| X-LAZY | Represent reduced values below `2p` and immediate linear intermediates below `4p`; use the same lazy runtime Edwards formulas as the pinned Ed301 arithmetic | Existing Ed301 5x64 limbs, product/reduction code and RFC 7748 ladder schedule | The private bound types serve both the unchanged Montgomery ladder and fixed-base Edwards path. X301-specific decoding, swap, inversion, output and zeroization remain separate. | Independent full-domain tests through bit 605, 5,000-point formula differential, Ed301 function-body provenance, X301 KAT/differential corpus, taint and final-provider codegen gates |
...
| X-RAND-INSTANCE | **Approved exception to E4 (2026-09-04).** X301 key generation uses one locked provider-owned `CTR-DRBG` with `AES-256-CTR`, created lazily and seeded by `RAND_get0_primary(child)` | OpenSSL `EVP_RAND(3)` and `RAND_get0_primary(3)` | `RAND_priv_bytes_ex(child)` creates thread-local child state whose safe teardown required per-operation cleanup. OpenSSL supplies the DRBG and parent hierarchy; the project owns selection, caching, locking and teardown. The child primary uses its OS seed source, not application `rand.seed` or `seed_strict`. An unsuitable parent fails with `PROV_R_PARENT_CANNOT_SUPPLY_ENTROPY_SEED`. A warm instance retains its implementation; a fresh context re-evaluates policy. | T7 teardown, policy-transition, deterministic, poisoned-RAND and dual-lane tests |
