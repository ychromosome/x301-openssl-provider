...
| E4 | No new RNG route | OpenSSL provider RAND contract (`EVP_RAND`, core `get_user_entropy`); existing provider keygen contract | Generate private material through one provider-owned OpenSSL DRBG instance fetched under the child-context policy; no project RNG, seed handling, or per-thread RAND state on the child context. Fixed-key derive consumes no RAND. |
